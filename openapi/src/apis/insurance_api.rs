...
pub async fn get_insurance_prices(configuration: &configuration::Configuration, accept_language: Option<&str>, datasource: Option<&str>, if_none_match: Option<&str>, language: Option<&str>) -> Result<Vec<models::GetInsurancePrices200Ok>, Error<GetInsurancePricesError>> {
    ...
    let uri_str = format!("{}/insurance/prices/", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::GetInsurancePrices200Ok&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::GetInsurancePrices200Ok&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetInsurancePricesError> = serde_json::from_str(&content).ok();
